...
def handle_form_submission(user_id):
    ...
    grouped_rooms = group_rooms_by_type(remaining_rooms)
    # Pass the data to the new template
    return render_template('customer_booking2.html', grouped_rooms=grouped_rooms, checkin_date=checkin_date, checkout_date=checkout_date, user_id=user_id)
...
@app.route('/save_selected_room', methods=['POST'])
def save_selected_room():
    if 'user_id' not in session:
    ...
    # Now you can save the selected room type to your database or perform any other necessary actions.

    flash(f"Room '{selected_room}' successfully booked!", "success")
