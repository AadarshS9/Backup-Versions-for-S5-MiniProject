...
with app.app_context():
    ...
    booking_data = [
        ...
        {"checkindate": "2023-12-05", "checkoutdate": "2023-12-15", "status": "cancelled", "duration": 10, "roomsid": 3, "custid": 3},
        {"checkindate": "2023-12-12", "checkoutdate": "2023-12-20", "status": "confirmed", "duration": 8, "roomsid": 4, "custid": 4},
        {"checkindate": "2023-12-01", "checkoutdate": "2023-12-10", "status": "cancelled", "duration": 9, "roomsid": 5, "custid": 5},
    ...
    payments_data = [
        {"paymenttype": "Credit Card", "amount": 500, "status": "completed", "completed": "Yes", "bookingid": 1, "custid": 1},
        {"paymenttype": "Debit Card", "amount": 400, "status": "completed", "completed": "Yes", "bookingid": 2, "custid": 2},
        {"paymenttype": "UPI", "amount": 300, "status": "completed", "completed": "Yes", "bookingid": 3, "custid": 3},
        {"paymenttype": "Credit Card", "amount": 450, "status": "completed", "completed": "Yes", "bookingid": 4, "custid": 4},
        {"paymenttype": "Debit Card", "amount": 350, "status": "completed", "completed": "Yes", "bookingid": 5, "custid": 5},
        {"paymenttype": "UPI", "amount": 250, "status": "completed", "completed": "Yes", "bookingid": 6, "custid": 6},
        # Add more payments data as needed
    ]

    # Sample data for cancellations
    cancellation_data = [
        {"date": "2023-12-25", "time": "14:30:00", "paymentid": 3, "bookingid": 3, "custid": 3},
        {"date": "2023-12-18", "time": "10:45:00", "paymentid": 5, "bookingid": 5, "custid": 5},
...
def admin_edit_room(room_id):
    if request.method == 'POST':
        # Handle the form submission to update the room in the database
    ...
    # Fetch the room details from the database based on room_id
    cursor = mysql.connection.cursor()
    ...
    # Render the edit room form with the room details
    return render_template('admin_edit_room.html', room_data=room_data)
...
def admin_add_room():
    ...
    if request.method == 'POST':
        # Handle the form submission to add a new room to the database
...
def admin_view_payments():
    ...
    # Fetch travel plans data from the database
    cursor = mysql.connection.cursor()
    ...
    # Define table headers for travel plans
    table_headers = ["Payment ID", "Payment Type", "Amount", "Status", "Completed", "Booking ID", "Customer ID"]

    # Convert travel plans data to a list of dictionaries
    payments_list = []
    for payments in payments_data:
        payments_dict = {
            ...
            "Booking ID": payments[5],
            "Customer ID": payments[6]
        }
    ...
    return render_template('admin_view_payments.html', headers=table_headers, payments_data=payments_json)
...
def admin_view_room_allocations():
    ...
    return render_template('admin_view_room_allocations.html', headers=table_headers, roomallocation_data=roomallocation_json)
...
def admin_view_customer():
    ...
    # Define table headers for travel plans
    table_headers = ["Customer ID", "Title", "First Name", "Last Name", "Email", "Phone no", "Country", "City", "SSN", "User ID"]
    ...
    return render_template('admin_view_customer.html', headers=table_headers, customer_data=customer_json)
...
def admin_view_registered_users():
    ...
    return render_template('admin_view_registered_users.html', headers=table_headers, users_data=users_json)
...
def admin_view_travel_plans():
    ...
    return render_template('admin_view_travel_plans.html', headers=table_headers, travel_plans_data=travel_plans_json)
...
def admin_view_cancellation():
    ...
    for cancellation in cancellation_data:
        cancellation_dict = {
            ...
            "Date": str(cancellation[1]),  # Convert date to string
            "Time": str(cancellation[2]),  # Convert time to string
    ...
    return render_template('admin_view_cancellation.html', headers=table_headers, cancellation_data=cancellation_json)
...
def customer_view_bookings(userid):
    ...
    return render_template('customer_view_bookings.html', headers=table_headers, custbookings_data=custbookings_json, user_id=user_id)
...
def customer_view_travelplan(userid):
    ...
    return render_template('customer_view_travelplan.html', headers=table_headers, custtravelplan_data=custtravelplan_json)
...
def customer_view_payments(userid):
    ...
    # Define table headers for bookings, including the new columns
    table_headers = ["Payment ID", "Payment Type", "Amount", "Status", "Completed", "Booking ID", "Customer ID", "Customer Name"]
    ...
    for custpayments in custpayments_data:
        custpayments_dict = {
            ...
            "Completed": custpayments[4],
            "Booking ID": custpayments[5],
            "Customer ID": custpayments[6],
            "Customer Name": f"{custpayments[7]} {custpayments[8]}"
        }
    ...
    return render_template('customer_view_payments.html', headers=table_headers, custpayments_data=custpayments_json)


@app.route('/customer/booking1/<int:userid>', methods=['GET', 'POST'])
def customer_booking1(userid):
    if 'user_id' not in session or session['user_id'] != userid:
        flash("You must be logged in as a user to view this page.", "error")
        return redirect(url_for('login'))
    ...
    return render_template('customer_booking1.html', user_id=user_id)
...
def handle_form_submission(user_id):
    ...
    return render_template('customer_booking2.html', grouped_rooms=grouped_rooms, checkin_date=checkin_date, checkout_date=checkout_date, duration=duration, user_id=user_id)
...
def customer_booking2(user_id):
   ...
   #     return redirect(url_for('login'))
    ...
    return render_template('customer_booking3.html', selected_room=selected_room, user_id=user_id,
                           checkin_date=checkin_date, checkout_date=checkout_date, duration=duration)
...
def customer_booking3(user_id):
    # Retrieve data from the form submission and session
    user_id = request.form.get('user_id')
    title = request.form.get('title')
    firstname = request.form.get('firstname')
    lastname = request.form.get('lastname')
    email = request.form.get('email')
    calling_code = request.form.get('calling_code')
    ...
    # You can store the customer details in the session for later use if needed
    ...
    return render_template('customer_booking4.html', user_id=user_id, selected_room=selected_room,
                           checkin_date=checkin_date, checkout_date=checkout_date, duration=duration,
                           title=title, firstname=firstname, lastname=lastname, email=email,
                           calling_code=calling_code, mobile_number=mobile_number, country=country, city=city)



@app.route('/customer/booking4', methods=['GET'])
def customer_booking4():
    ...
    calling_code = session.get('calling_code')
    ...
    if not all([user_id, selected_room, checkin_date, checkout_date, duration,
                title, firstname, lastname, email, calling_code, mobile_number, country, city]):
        flash("Missing information. Please start the booking process again.", "error")
